...
class HrPayslip(models.Model):
    ...
    def _check_dates(self):
        if any(
            self.filtered(lambda payslip: payslip.date_from > payslip.date_to)
        ):
    ...
    def unlink(self):
        if any(
            ...
            raise UserError(
                _(
                    "You cannot delete a payslip which is not draft or "
                    "cancelled!"
                )
            )
...
class HrPayslipLine(models.Model):
    ...
    rate = fields.Float(
        string="Rate (%)", digits="Payroll Rate", default=100.0
    )
