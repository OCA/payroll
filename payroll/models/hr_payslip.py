...
class HrPayslip(models.Model):
    ...
    struct_id = fields.Many2one(
        ...
        help='Defines the rules that have to be applied to this payslip, '
             ...
             'contract, this field isn\'t mandatory anymore and thus the '
             'rules applied will be all the rules set on the structure of all '
             'contracts of the employee valid for the chosen period')
    name = fields.Char(
        string='Payslip Name', readonly=True,
        states={'draft': [('readonly', False)]})
    number = fields.Char(
    ...
    # this is chaos: 4 states are defined, 3 are used ('verify' isn't) and
    # 5 exist ('confirm' seems to have existed)
    ...
    line_ids = fields.One2many(
        'hr.payslip.line', 'slip_id', string='Payslip Lines', readonly=True,
        states={'draft': [('readonly', False)]})
    ...
    paid = fields.Boolean(
        ...
        states={'draft': [('readonly', False)]})
    note = fields.Text(
        string='Internal Note', readonly=True,
        states={'draft': [('readonly', False)]})
    contract_id = fields.Many2one(
        'hr.contract', string='Contract', readonly=True,
        states={'draft': [('readonly', False)]})
    details_by_salary_rule_category = fields.One2many(
        'hr.payslip.line', compute='_compute_details_by_salary_rule_category',
    ...
    payslip_run_id = fields.Many2one(
        'hr.payslip.run', string='Payslip Batches', readonly=True,
    ...
    def _compute_details_by_salary_rule_category(self):
        for payslip in self:
            payslip.details_by_salary_rule_category = payslip\
                .mapped('line_ids').filtered(lambda line: line.category_id)
    ...
    def _check_dates(self):
        if any(self.filtered(
            ...
            raise ValidationError(_(
                "Payslip 'Date From' must be earlier 'Date To'."))
    ...
    def unlink(self):
        if any(self.filtered(
            ...
            raise UserError(_(
                'You cannot delete a payslip which is not draft or cancelled!'
            ))
    ...
    def compute_sheet(self):
        for payslip in self:
            number = payslip.number or self.env['ir.sequence']\
                .next_by_code('salary.slip')
        ...
        for contract in contracts.filtered(
            ...
            for day, hours, leave in day_leave_intervals:
                ...
                current_leave_struct = leaves.setdefault(
                    ...
                    {
                        'name': holiday.holiday_status_id.name or
                        _('Global Leaves'),
                        'sequence': 5,
    ...
    # YTI TODO To rename. This method is not really an onchange, as it is not
    # in any view employee_id and contract_id could be browse records
    def onchange_employee_id(
        ...
        res = {
            'value': {
                ...
                [(2, x,) for x in self.worked_days_line_ids.ids],
    ...
    def create(self, vals_list):
        for values in vals_list:
            if 'employee_id' not in values or 'contract_id' not in values:
                ...
                values['employee_id'] = values.get('employee_id') or \
...
class HrPayslipInput(models.Model):
    ...
    amount = fields.Float(
        help="It is used in computation. For e.g. A rule for sales having "
             ...
             "expression like result = inputs.SALEURO.amount * "
             "contract.wage*0.01.")
    contract_id = fields.Many2one(
        'hr.contract', string='Contract', required=True,
        help="The contract for which applied this input")
...
class HrPayslipRun(models.Model):
    ...
    date_start = fields.Date(
        string='Date From', required=True, readonly=True,
        states={'draft': [('readonly', False)]},
        default=lambda self: fields.Date.to_string(date.today().replace(day=1))
    )
    date_end = fields.Date(
        string='Date To', required=True, readonly=True,
        states={'draft': [('readonly', False)]},
        default=lambda self: fields.Date.to_string((
