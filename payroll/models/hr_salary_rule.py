...
class HrPayrollStructure(models.Model):
    ...
    rule_ids = fields.Many2many(
        'hr.salary.rule', 'hr_structure_salary_rule_rel', 'struct_id',
    ...
    def get_all_rules(self):
        """
        @return: returns a list of tuple (id, sequence) of rules that are
                 maybe to apply
        """
        all_rules = []
        for struct in self:
...
class HrSalaryRuleCategory(models.Model):
    ...
    parent_id = fields.Many2one(
        'hr.salary.rule.category', string='Parent',
    ...
    def _check_parent_id(self):
        if not self._check_recursion():
            raise ValidationError(_(
                'Error! You cannot create recursive hierarchy of Salary Rule '
                'Category.'))
...
class HrSalaryRule(models.Model):
    ...
    condition_python = fields.Text(
        ...
        default='''
                    ...
                    # rules: object containing the rules code (previously
                      computed)
                    # categories: object containing the computed salary rule
                      categories (sum of amount of all rules belonging to that
                      category).
    ...
    child_ids = fields.One2many(
        'hr.salary.rule', 'parent_rule_id', string='Child Salary Rule',
        copy=True)
        ...
        if self.amount_select == 'fix':
            try:
                return self.amount_fix, \
                    float(safe_eval(self.quantity, localdict)), 100.0
            except Exception:
                raise UserError(_(
            ...
            try:
                ...
                raise UserError(_(
                    'Wrong percentage base or quantity defined for salary rule'
                    ' %s (%s).') % (self.name, self.code))
        else:
            try:
                safe_eval(
                    self.amount_python_compute, localdict, mode='exec',
                    nocopy=True)
                return float(localdict['result']), \
                    'result_qty' in localdict and localdict['result_qty'] or \
                    1.0, 'result_rate' in localdict and \
