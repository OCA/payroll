...
class TestPayslipBase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.pf_rule_id = self.ref(
            'payroll.hr_salary_rule_providentfund1')
        self.mv_rule_id = self.ref(
            'payroll.hr_salary_rule_meal_voucher')
        self.comm_rule_id = self.ref(
            'payroll.hr_salary_rule_sales_commission')
        ...
        self.developer_pay_structure = self.env['hr.payroll.structure'].\
            create({
